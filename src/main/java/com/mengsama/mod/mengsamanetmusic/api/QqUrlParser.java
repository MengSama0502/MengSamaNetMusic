...
public final class QqUrlParser {
    ...
    public static ParsedUrl parse(String input) {
        ...
        if (SONG_MID_ONLY.matcher(input).matches()) {
            return new ParsedUrl(ParsedUrl.ResourceType.SONG, input);
        }
        ...
        if (m.find()) {
            return new ParsedUrl(ParsedUrl.ResourceType.ALBUM, m.group(1));
        }

        if (url.contains("songDetail") || url.contains("song/")) {
            m = SONG_DETAIL.matcher(url);
            if (m.find()) {
                return new ParsedUrl(ParsedUrl.ResourceType.SONG, m.group(1));
            }
        }
        ...
        if (url.contains("playlist")) {
            ...
            if (m.find()) {
                return new ParsedUrl(ParsedUrl.ResourceType.PLAYLIST, m.group(1));
            }
        }
        ...
        if (url.contains("taoge.html") || url.contains("playsong.html")) {
            ...
            if (m.find()) {
                ...
                if (url.contains("taoge")) {
                    return new ParsedUrl(ParsedUrl.ResourceType.PLAYLIST, id);
                }
